...
$ticketsAvailable = appTableExists($pdo, 'tickets');
$ticketMessagesAvailable = appTableExists($pdo, 'ticket_messages');
$ticketAttachmentsAvailable = appTableExists($pdo, 'ticket_attachments');
...
if ($currentTab === 'logs' && !in_array($_SESSION['role'], ['admin', 'staff'], true)) {
    $currentTab = 'faq';
}
// =========================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'add_source') {
        ...
        if (!$ticketsAvailable || !$ticketMessagesAvailable) {
            header("Location: admin_dashboard.php?tab=tickets&ticket_error=missing_tables");
            exit;
        }
...
<body>
    ...
    <div class="main-content">
        <div class="header">
            ...
            <div class="admin-profile-container" style="position: relative;">
                <div class="admin-profile" onclick="toggleAdminMenu()">
                    ...
                    <div class="avatar"><?php $n = trim($_SESSION['ho_ten'] ?? 'A');
                                        echo htmlspecialchars(mb_strtoupper(mb_substr($n, mb_strpos($n, ' ') !== false ? mb_strrpos($n, ' ') + 1 : 0, 1, 'UTF-8'), 'UTF-8'), ENT_QUOTES, 'UTF-8'); ?></div>
                </div>
                <!-- Dropdown menu -->
        ...
        <div id="tab-dashboard" class="tab-content <?php echo $currentTab === 'dashboard' ? 'active' : ''; ?>">
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="stat-title">Tổng số Ticket</div>
                    <div class="stat-value"><?php echo $totalTickets; ?></div>
                </div>
                <div class="stat-card danger">
                    <div class="stat-title">Ticket chờ xử lý</div>
                    <div class="stat-value"><?php echo $pendingTickets; ?></div>
                </div>
                <div class="stat-card success">
                    <div class="stat-title">Ticket đã phản hồi</div>
                    <div class="stat-value"><?php echo $repliedTickets; ?></div>
                </div>
                <div class="stat-card">
                    <div class="stat-title">Tri thức đã verified</div>
                    <div class="stat-value"><?php echo $totalFaq; ?></div>
                </div>
            </div>
        ...
        <div id="tab-tickets" class="tab-content <?php echo $currentTab === 'tickets' ? 'active' : ''; ?>">
            ...
            <?php if (!$ticketsAvailable || !$ticketMessagesAvailable): ?>
                <div class="table-container" style="margin-bottom:16px;border-left:4px solid #f79009;">
                    <strong>Module Ticket chưa sẵn sàng.</strong>
                    <p style="margin:8px 0 0;color:var(--text-muted);">
                        Database đang thiếu bảng <code>tickets</code> hoặc <code>ticket_messages</code>.
                        Tab Lịch sử Chat vẫn có thể sử dụng bình thường; hãy import script tạo bảng Ticket khi cần bật module này.
                    </p>
                </div>
            <?php endif; ?>
        ...
        <?php if (in_array($_SESSION['role'], ['admin', 'staff'], true)): ?>
            <div id="tab-logs" class="tab-content <?php echo $currentTab === 'logs' ? 'active' : ''; ?>">
                <div class="page-title chat-log-page-title">
                    <div>
                        <span>Lịch sử trò chuyện Sinh viên - Bot</span>
                        <div class="chat-log-subtitle">Dữ liệu được đọc từ chat_sessions và chat_messages.</div>
                    </div>
                    <button type="button" class="btn btn-outline" id="chatLogRefresh">Làm mới</button>
                </div>

                <form id="chatLogFilterForm" class="chat-log-filters">
                    <div class="chat-log-field chat-log-field-wide">
                        <label for="chatLogKeyword">Tìm kiếm</label>
                        <input id="chatLogKeyword" type="search" placeholder="MSSV, họ tên, tiêu đề hoặc UUID...">
                    </div>
                    <div class="chat-log-field">
                        <label for="chatLogStatus">Trạng thái</label>
                        <select id="chatLogStatus">
                    ...
                    <div class="chat-log-field">
                        <label for="chatLogIntent">Intent</label>
                        <input id="chatLogIntent" type="text" placeholder="Ví dụ: personal_schedule">
                    </div>
                    <div class="chat-log-field">
                        <label for="chatLogFromDate">Từ ngày</label>
                        <input id="chatLogFromDate" type="date">
                    </div>
                    <div class="chat-log-field">
                        <label for="chatLogToDate">Đến ngày</label>
                        <input id="chatLogToDate" type="date">
                    </div>
                    <div class="chat-log-filter-actions">
                        <button type="submit" class="btn">Lọc dữ liệu</button>
                        <button type="button" class="btn btn-outline" id="chatLogReset">Đặt lại</button>
                    </div>
                </form>

                <div class="chat-log-toolbar">
                    <strong id="chatLogTotal">0 phiên chat</strong>
                    <span>Chỉ Admin và Staff được phép truy cập.</span>
                </div>

                <div class="table-container chat-log-table-container">
                    <div class="chat-log-table-scroll">
                        <table class="chat-log-table">
                            <thead>
                                <tr>
                                    <th>STT</th>
                                    <th>Sinh viên</th>
                                    <th>Phiên trò chuyện</th>
                                    <th>Tin nhắn</th>
                                    <th>Trạng thái</th>
                                    <th>Intent gần nhất</th>
                    ...
                    <div class="chat-log-pagination">
                        <button type="button" class="btn btn-outline btn-sm" id="chatLogPrev">Trang trước</button>
                        <span id="chatLogPageInfo">Trang 1/1</span>
                        <button type="button" class="btn btn-outline btn-sm" id="chatLogNext">Trang sau</button>
                    </div>
                </div>
            </div>
        <?php endif; ?>
    ...
    <div class="modal-overlay" id="chatSessionModal" aria-hidden="true">
        <div class="modal-box chat-session-modal-box" role="dialog" aria-modal="true" aria-labelledby="chatSessionModalTitle">
            <button type="button" class="modal-close-btn" data-close-chat-session-modal aria-label="Đóng">&times;</button>
            <div class="chat-session-modal-header">
                <h3 id="chatSessionModalTitle">Chi tiết cuộc trò chuyện</h3>
                <p id="chatSessionModalStudent"></p>
            </div>
            <div id="chatSessionModalMeta" class="chat-session-modal-meta"></div>
            <div id="chatSessionMessages" class="chat-session-message-list">
                <div class="chat-log-loading">Đang tải nội dung phiên chat...</div>
            </div>
        </div>
        <button type="button" class="chat-session-modal-backdrop" data-close-chat-session-modal aria-label="Đóng cửa sổ"></button>
    </div>
    ...
    <script>
        function switchTab(tabId, element) {
            ...
            if (tabId === 'logs' && window.uthChatHistory) {
                window.uthChatHistory.loadSessions(1);
            }
        }
    ...
    <div class="modal-overlay" id="studentDetailModal">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top: 15px;">
            <div class="info-item">
                <div class="info-label" style="color: gray; font-size: 13px;">MSSV</div>
                <div id="dt_mssv" style="font-weight: bold; font-size: 15px;"></div>
            </div>
            <div class="info-item">
                <div class="info-label" style="color: gray; font-size: 13px;">Họ và tên</div>
                <div id="dt_hoten" style="font-weight: bold; font-size: 15px;"></div>
            </div>
            <div class="info-item">
                <div class="info-label" style="color: gray; font-size: 13px;">Ngày sinh</div>
                <div id="dt_ngaysinh" style="font-weight: bold; font-size: 15px;"></div>
            </div>
            <div class="info-item">
                <div class="info-label" style="color: gray; font-size: 13px;">Nơi sinh</div>
                <div id="dt_noisinh" style="font-weight: bold; font-size: 15px;"></div>
            </div>
            <div class="info-item">
                <div class="info-label" style="color: gray; font-size: 13px;">Giới tính</div>
                <div id="dt_gioitinh" style="font-weight: bold; font-size: 15px;"></div>
            </div>
            <div class="info-item">
                <div class="info-label" style="color: gray; font-size: 13px;">Khóa học</div>
                <div id="dt_khoahoc" style="font-weight: bold; font-size: 15px;"></div>
            </div>
            <div class="info-item">
                <div class="info-label" style="color: gray; font-size: 13px;">Bậc đào tạo</div>
                <div id="dt_bacdaotao" style="font-weight: bold; font-size: 15px;"></div>
            </div>
            <div class="info-item">
                <div class="info-label" style="color: gray; font-size: 13px;">Loại hình</div>
                <div id="dt_loaihinh" style="font-weight: bold; font-size: 15px;"></div>
            </div>
            <div class="info-item" style="grid-column: span 2;">
                <div class="info-label" style="color: gray; font-size: 13px;">Ngành học</div>
                <div id="dt_nganh" style="font-weight: bold; font-size: 15px;"></div>
            </div>
            <div class="info-item" style="grid-column: span 2;">
                <div class="info-label" style="color: gray; font-size: 13px;">Chuyên ngành</div>
                <div id="dt_chuyennganh" style="font-weight: bold; font-size: 15px;"></div>
            </div>
        </div>
    ...
    <div id="adminProfileModal" class="modal-overlay">
        <div class="modal-box" style="max-width: 400px; text-align: center;">
            ...
            <div class="avatar" style="width: 80px; height: 80px; font-size: 32px; margin: 0 auto 20px;">
                <?php $n = trim($_SESSION['ho_ten'] ?? 'A');
                echo htmlspecialchars(mb_strtoupper(mb_substr($n, mb_strpos($n, ' ') !== false ? mb_strrpos($n, ' ') + 1 : 0, 1, 'UTF-8'), 'UTF-8'), ENT_QUOTES, 'UTF-8'); ?>
            </div>
    ...
    <script src="../js/admin_chat_history.js"></script>
</body>
